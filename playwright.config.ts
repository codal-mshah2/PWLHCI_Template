...
const reportConfig: OrtoniReportConfig = {
  ...
  logo: "gocodal.jpg",
  title: "PlayPOM Template Project Execution Report",
  showProject: true,
  projectName: "PlayPOM Template Project",
  testType: "e2e",
...
export const playPOMConfig: PlayPOMConfig = {
  ...
  sendEmailAddresses: [],
  environmentNameForEmailSubject: process.env.ENVIRONMENT_NAME || "LOCAL",
};
...
export default defineConfig({
  ...
  testIgnore: ["LambdaTestSuite/RegisterSuite.spec.ts"],
  ...
  use: {
    ...
    launchOptions: {
      slowMo: 500,
      // run in headed mode so we can see the browser while tests run
    ...
    colorScheme: "light",
    // record video of the test runs (will store per-test videos in the configured outputDir)
    video: "on",
